...
; --- 1) API pra retornar teclados (lista de IDs / Indexes) ---
ids := []
try {
...
if (IsObject(ids) && ids.Length > 0) {
    ...
    ; --- 2) Fallback (pode devolver ints ou objetos) ---
    devs := []
    try {
...
fh.WriteLine("Lista de teclados (AHI) — use keyboard_index ou keyboard_id no autoFocus.ini")
fh.WriteLine("---------------------------------------------------------------")
