...
sub get_ntp_response {
    ...
    if (HAVE_SOCKET_INET6 && !$is_ipv4) {
        eval {
            $sock = IO::Socket::INET6->new(%args);
        };
        if ($@ || !$sock) {
            warn "[TOTP] IO::Socket::INET6 falhou para $host, tentando INET: $@\n";
            undef $sock;
        } else {
            warn "[TOTP] Usando IO::Socket::INET6 para $host\n";
        }
    }
    ...
    if (!$sock) {
        ...
        if ($@ || !$sock) {
            die "[TOTP] Falha ao criar socket para $host: $@\n";
        } else {
            warn "[TOTP] Usando IO::Socket::INET para $host\n";
        }
    }
    ...
    $sock->send($packet->encode)
        or die "[TOTP] send() falhou para $host: $!\n";

    my $data;
    eval {
        local $SIG{ALRM} = sub { die "Net::NTP timeout\n"; };
        alarm($TIMEOUT);
        my $recv_ok = $sock->recv($data, 960);
        if (!$recv_ok || !defined($data) || length($data) < 48) {
            die "[TOTP] recv() falhou: $! (host $host, data: " . (defined($data) ? length($data) . " bytes" : "undef") . ")\n";
        }
        alarm(0);
    };
    alarm 0;
    ...
    my %packet;
    for (my $i = 0; $i < @ntp_packet_fields; $i++) {
